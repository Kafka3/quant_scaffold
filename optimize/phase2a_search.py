...
Usage:
    python optimize/phase2a_search.py --config configs/base.yaml --data data/raw/BTCUSDT_5m_2024.csv
"""
...
def main() -> None:
    ...
    if len(baseline) > 0:
        ...
        print(f"   return={b['full_year_total_return']:.2f}% trades={b['full_year_total_trades']:.0f} "
              f"pf={safe_profit_factor(b['full_year_profit_factor']):.2f} q+={b['q_positive_count']:.0f}/4 "
    ...
    if len(baseline) > 0:
        ...
        beaters = summary_df[
            ...
            & (summary_df["full_year_profit_factor"] >= safe_profit_factor(b["full_year_profit_factor"]))
